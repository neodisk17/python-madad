...
suite('downloadPackages Test Suite', () => {
  let showWarningMessageStub: sinon.SinonStub, activeTextEditorStub: sinon.SinonStub;
  ...
  test("If an active editor is not available", async () => {
    ...
    assert.ok(showWarningMessageStub.calledOnceWith(downloadPackagesError.noActiveEditor));
  ...
  test('Check if the command was run on the terminal', async () => {
    ...
    activeTextEditorStub.get(() => ({ document: { fileName: "requirement.txt" } }));
    ...
    if (pythonExtension) {
      ...
      if (pythonPath) {
        ...
        assert.ok(mockTerminal.sendText.calledOnceWithExactly(sinon.match('python -m pip install -r requirement.txt'), sinon.match(1)));
        assert.ok(!mockTerminal.sendText.calledOnceWithExactly(sinon.match('python -m pip install -r requirement.txt'), sinon.match(0)));
